...
export default function ProcessStrengthSection() {
  ...
  return (
    <div ref={containerRef} className="relative h-[300vh]">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center bg-[#EDF3FF]">
              ...
              {hasEntered && currentSection === 'process' && (
                ...
                >
                  ...
                  <motion.div
                    ...
                    className="bg-[#1B67FF] text-white text-center py-3 origin-center"
                  >
                  ...
                  >
                    {PROCESS_CARDS.map((card, index) => (
                      <motion.div
                        ...
                        className={`bg-white rounded-[24px] p-[35px] ${
                      ...
                      >
                        ...
                        <div className="flex justify-between items-start mb-[46px]">
                          ...
                          <span
                            className="text-[48px] md:text-[64px] leading-none text-[#E0EBFF]"
                        ...
                        {/* Title */}
                        <h3 className="text-[20px] md:text-[28px] font-bold text-[#111111] mb-[46px]">
                          {card.title}
                        </h3>

                        {/* Description */}
                        <p className="text-[13px] md:text-[16px] text-[#727783] leading-relaxed whitespace-pre-line">
                          {card.description}
                        </p>
                      </motion.div>
                    ))}
                  ...
                  >
                    {STRENGTH_CARDS.map((card, index) => (
                      <motion.div
                        key={card.id}
                        variants={cardVariants}
                        className={`bg-white rounded-[24px] py-[69px] px-[35px] ${
                          index === 2 ? 'col-span-2 lg:col-span-1' : ''
                        }`}
                      >
                        {/* Label */}
                        <div className="mb-4">
                          <span className="inline-block px-3 md:px-4 py-0.5 md:py-0.5 bg-[#E0EBFF] text-[#1B67FF] text-[12px] md:text-[13px] font-medium rounded-full">
                            {card.label}
                          </span>
                        </div>
                        ...
                        {/* Value */}
                        <div className="mb-2">
                          <span className="text-[36px] md:text-[56px] font-bold text-[#1B67FF]">
                            {card.value}
                          </span>
                          <span className="text-[14px] md:text-[20px] text-[#1B67FF] ml-1 md:ml-2">
                            {card.unit}
                          </span>
                        </div>

                        {/* Title */}
                        <h3 className="text-[16px] md:text-[22px] text-[#1B67FF] mb-2 md:mb-4">
                          {card.title}
                        </h3>

                        {/* Description */}
                        <p className="text-[13px] md:text-[16px] text-[#97BAFF] leading-relaxed whitespace-pre-line">
                          {card.description}
                        </p>
                      </motion.div>
                    ))}
